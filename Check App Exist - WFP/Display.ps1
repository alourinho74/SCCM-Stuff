...
Function Get_App
{
    ...
    Write-Host $app_name -ForegroundColor Magenta
        ...
        if ($strHostname -ne "")
        {
            if (Test-Connection -count 1 -Quiet $strHostname)
            {
                try 
                {
                    ...
                    Write-Host $app_name
                    $app_sign = Get-WmiObject -computername $strHostname -Namespace "ROOT\cimv2\sms" -Query "Select ArpDisplayname,ProductVersion,InstallDate From sms_installedSoftware where ArpDisplayname like '%$app_name%'"

                    $aux = $app_sign
                
                    Write-Host ($app_sign).count  -ForegroundColor Green
                }
                catch 
                {
                    Write-Host "catch"
