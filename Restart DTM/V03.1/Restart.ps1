...
Function handelEventlogSource
{
    ...
    Write-Host "Evenlog Application? " + $ApplicationEventLogExists
    Write-Host "Source ? " + $MyEventLogSourceExists
...
function handelTPM
{
    ...
    try
    {
        ...
        if ($os.caption -eq "Microsoft Windows 7 Professional ")
        {
            Dolog -Message 'Windows 7 Professional detect. Skip TPM Check.' -color "White"
            $aux_tpm = $true
        }
        else
        {
            try
            {
                ...
                {
                    dolog -Message 'TPM not present on this hardware. Check Bitlocker Status...' -LogLevel 2 -color "Yellow"
                    $aux_tpm = $false
                }
            }
            catch
            {
                dolog -Message 'Unable to query TPM status. TPM does not exist or could not be reached. Continue....' -LogLevel 2 -color "Red"
                return $false
            }
        }
    }
    ...
    {
        $ExitRC = 3
        Dolog -Message 'Unable to check operating system..Possible WMI Error' -LogLevel 3 -color "White"
    ...
    if ($aux_tpm -eq $false)
    {
        try
        {
            $bitlockerstatus = Get-BitLockerVolume -MountPoint  "$($os.systemdrive)"

            if ($bitlockerstatus.protectionstatus -eq $true)
            {
                try
                {
            ...
            {
                dolog -Message 'Bitlocker is not protecting system drive ' -color "Yellow"
            }
        }
        catch
        {
            dolog -Message 'Unable to query Bitlocker Status' -LogLevel 2 -color "Red"
        }
