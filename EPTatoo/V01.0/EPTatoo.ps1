    ...
    Try 
    {
        ...
        try 
        {
             ...
             if (-not $aux)
             {
                 ...
                 Write-Host "InstalledBeginTime = " $InstalledBeginTime
                 ...
                 try
                 {
                    Set-WmiInstance -Path \\.\root\cimv2:eptatoo -Arguments @{AdvertisementID=$AdvertisementID;ClientVersion=$ClientVersion;Computername=$Computername;InstallationType=$InstallationType;InstalledBeginTime=$InstalledBeginTimeGMT;InstalledEndTime=$InstalledEndTimeGMT;MediaType=$MediaType;Organization=$Organization;PrevOperatingSystem=$PrevOperatingSystem;TaskSequenceName=$TaskSequenceName;TaskSequenceversion=$TaskSequenceversion;TSSequence=$TSSequence;TSSequenceTime=$TSSequenceTimeGMT}
